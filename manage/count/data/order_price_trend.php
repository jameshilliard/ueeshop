...
$count_rs=$db->query("select FROM_UNIXTIME(OrderTime, '{$count_type_ary[$CountType]}') as OrderDate, sum(TotalPrice) as TotalPrice from orders where $where group by OrderDate");
$data_ary=array();
while($count_row=mysql_fetch_assoc($count_rs)){
	$data_ary[$count_row['OrderDate']]=$count_row['TotalPrice'];
}

if($CountType==0){	//按天
	for($i=$time_start; $i<=$time_end; $i+=3600*24){
		echo date('Y-m-d', $i).';'.(float)$data_ary[date('Y-m-d', $i)]."\r\n";
	}
}elseif($CountType==1){	//按月
	...
	for($i=$s; $i<=$e; $i++){
		...
		echo $d.';'.(float)$data_ary[$d]."\r\n";
	}
}else{	//按年
	$s=date('Y', $time_start);
	$e=date('Y', $time_end);
	for($i=$s; $i<=$e; $i++){
		echo $i.';'.(float)$data_ary[$i]."\r\n";
	}
}
?>