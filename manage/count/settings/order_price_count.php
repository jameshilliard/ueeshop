...
<settings> 
  ...
  <column>
    ...
    <width>40</width>                                         <!-- [80] (0 - 100) width of column (in percents)  -->
    ...
    <sequenced_grow>false</sequenced_grow>                     <!-- [false] (true / false) whether columns should grow at the same time or one after another -->
    <alpha>80</alpha>                                        <!-- [100] (Number) alpha of all columns -->
    ...
    <balloon_text>                                                    
     <![CDATA[Anomaly in {series}: {value}C]]>                                            <!-- [] ({title} {value} {series} {percents} {start} {difference} {total}) You can format any data label: {title} will be replaced with real title, {value} - with value and so on. You can add your own text or html code too. -->
    </balloon_text>    
    ...
    <corner_radius_top>8</corner_radius_top>                   <!-- [0] (Number, Number%) Corner radius of the column's top. Works only if depth is = 0 -->
  ...
  <line>                                                      <!-- Here are general settings for "line" graph type. You can set most of these settings for individual lines in graph settings below -->
    ...
    <data_labels_text_size></data_labels_text_size>           <!-- [text_size] (Number) -->
    <balloon_text>                                                    
      <![CDATA[Anomaly in {series}: {value}C (Smoothed)]]>                                           <!-- [] use the same formatting rules as for data labels -->
    </balloon_text>      
    <link_target></link_target>                               <!-- [] (_blank, _top ...) -->
  </line>
  ...
  <plot_area>                                                 <!-- PLOT AREA (the area between axes) -->
    ...
    <margins>                                                 <!-- plot area margins -->
      ...
      <bottom>50</bottom>                                     <!-- [80](Number / Number%) -->
    </margins>
  </plot_area>
  ...
  <labels>                                                    <!-- LABELS -->
    ...
    <label lid="0">
      ...
      <text>                                                  <!-- [] (text) html tags may be used (supports <b>, <i>, <u>, <font>, <a href="">, <br/>. Enter text between []: <![CDATA[your <b>bold</b> and <i>italic</i> text]]>-->
        <![CDATA[<b><?=get_lang('count.order_price_count');?></b>]]>
      </text>        
    </label>
  </labels>
  ...
  <graphs>                                                    <!-- GRAPHS SETTINGS. These settings can also be specified in data file, as attributes of <graph>, in this case you can delete everything from <graphs> to </graphs> (including) -->
    ...
    <graph gid="1">                                           <!-- if you are using XML data file, graph "gid" must match graph "gid" in data file -->
      ...
      <title>Visits</title>                                  <!-- [] (graph title) -->
      <color>B92F2F</color>                                   <!-- [] (hex color code)  -->
      <alpha></alpha>                                         <!-- [column.alpha (line.alpha)] (0 - 100) -->
      <data_labels>
        <![CDATA[<?=get_lang('ly200.price_symbols');?>{value}]]>                                          <!-- [column.data_labels (line.data_labels)] ({title} {value} {series} {percents} {start} {difference} {total}) You can format any data label: {title} will be replaced with real title, {value} - with value and so on. You can add your own text or html code too. -->
      </data_labels>      
      ...
      <balloon_text>
        <![CDATA[{series}: <?=get_lang('ly200.price_symbols');?>{value}]]>                         <!-- [column(line).balloon.text] ({title} {value} {series} {description} {percents}) You can format any balloon text: {title} will be replaced with real title, {value} - with value and so on. You can add your own text or html code too. -->
      </balloon_text>       
