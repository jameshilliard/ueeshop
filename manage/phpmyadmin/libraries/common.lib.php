...
if (!isset($theme_name, $theme_generation, $theme_version))
    $GLOBALS['theme'] = 'default'; // invalid theme

if ($theme_generation != PMA_THEME_GENERATION)
    $GLOBALS['theme'] = 'default'; // different generation

if ($theme_version < PMA_THEME_VERSION)
    $GLOBALS['theme'] = 'default'; // too old version
...
if (!is_dir($pmaThemeImage)) {
    $pmaThemeImage = $cfg['ThemePath'] . '/default/img/';
}
// end theme manager
...
// 2004-05-04: replaced with a modified function from Michael Keck (mkkeck)
                            ...
                            $exit = TRUE)
    {
        ...
        $tmp_mysql_error = ''; // for saving the default $error_message
        if (!empty($error_message)) {
            $tmp_mysql_error = strtolower($error_message); // save the default $error_message
        ...
        // To preserve default formatting, but allow wordwrapping, we do a couple of replacements
...
<?php echo $strPmaUriError;?><br /><br />
     ...
     *                   current MySQL release is >= 3.23.6, the default one
     *                   else
     *
     * @access  public
     */
...
<div align="<?php echo $GLOBALS['cell_align_left'];?>">
    ...
    <tr>
        <th<?php echo ($GLOBALS['theme'] != 'default') ? ' class="tblHeaders"' : ' bgcolor="' . $cfg['ThBgcolor'] . '"';?>>
