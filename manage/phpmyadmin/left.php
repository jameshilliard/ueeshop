...
function PMA_nestedSetHeaderParent($baseid, $key, $keyhistory, $indent, $indent_level, $val, $childout = true) {
    ...
    echo str_repeat(' ', $indent * 6) . '<div class><img src="' . $GLOBALS['pmaThemeImage'] . 'spacer.png' . '" border="0" width="' . (($indent - 1) * $indent_level) . '" height="9" alt="" /><a class="item" href="' . $GLOBALS['cfg']['DefaultTabDatabase'] . '?' . $GLOBALS['common_url_query'] . '&amp;tbl_group=' . htmlspecialchars($groupkey) . '" onclick="if (capable) {expandBase(\'el' . $id . '\', true); return false} else {return true}">';
        ...
        // list of tables contained in the current database
            ...
            if (!empty($db_start) && $db == $db_start) {
                ...
                while (@list($table, $type) = PMA_DBI_fetch_row($tables)) {
                    ...
                    $table_array[$table] .= '    <div class><a target="phpmain' . $hash . '" href="sql.php?' . $common_url_query . '&amp;table=' . urlencode($table) . '&amp;sql_query=' . (isset($book_sql_query) && $book_sql_query != FALSE ? urlencode($book_sql_query) : urlencode('SELECT * FROM ' . PMA_backquote($table))) . '&amp;pos=0&amp;goto=' . $cfg['DefaultTabTable'] . '">' . "\n";
...
// Case where only one database has to be displayed
    ...
    if (!$cfg['LeftFrameLight']) {
            ...
            if (!empty($num_tables)) {
                echo "\n";
                ?>
        <div class><a class="item" href="<?php echo $cfg['DefaultTabDatabase'];?>?<?php echo $common_url_query;?>" onclick="if (capable) {expandBase('el2', true); return false} else {return true}">
            <img name="imEx" id="el2Img" src="<?php echo $pmaThemeImage;?>b_plus.png" border="0" width="9" height="9" alt="+" /></a>
                <?php
            } else {
                echo "\n";
                ?>
        <div class><img name="imEx" src="<?php echo $pmaThemeImage;?>b_minus.png" border="0" width="9" height="9" alt="-" />
                <?php
            }
            echo "\n";
            ?>
