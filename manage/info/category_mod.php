...
check_permit('info_category', 'info.category.mod');
...
if($_POST){
	...
	$SubUId=get_UId_by_CateId($CateId, 'info_category');	//下级类别的UId
	$CurUId=$db->get_value('info_category', "CateId='$CateId'", 'UId');
	$db->query("update info_category set UId=replace(UId, '$CurUId', '$UId') where UId like '{$SubUId}%'");
	...
	save_manage_log('更新文章类别:'.$Category);
...
$category_description_row=$db->get_one('info_category_description', "CateId='$CateId'");

include('../../inc/manage/header.php');
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="category.php"><?=get_lang('info.category_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.mod');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	...
	<?php }?>
	<?php
	if(get_cfg('info.category.dept')>1){
		$o_dept=$category_row['Dept']-($db->get_max('info_category', "UId like '{$category_row['UId']}{$category_row['CateId']},%'", 'Dept')-get_cfg('info.category.dept'));	//剩余可用的级数
		$e_where="CateId!='{$category_row['CateId']}' and Dept<".($category_row['SubCate']?$o_dept:get_cfg('info.category.dept'));
	?>
		<tr>
			<td nowrap><?=get_lang('ly200.under_the');?>:</td>
			<td><?=ouput_Category_to_Select('UnderTheCateId', get_CateId_by_UId($category_row['UId']), 'info_category', "UId='0,' and $e_where", $e_where, get_lang('ly200.select'));?></td>
		</tr>
	<?php }?>
	...
	<tr>
		<td>&nbsp;</td>
		<td><input type="submit" name="submit" class="form_button" value="<?=get_lang('ly200.mod');?>"><a href='category.php' class="return"><?=get_lang('ly200.return');?></a><input type="hidden" name="CateId" value="<?=$CateId;?>"></td>
	</tr>
</table>
</form>
