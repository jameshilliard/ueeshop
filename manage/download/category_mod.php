...
check_permit('download_category', 'download.category.mod');
...
if($_POST){
	...
	$SubUId=get_UId_by_CateId($CateId, 'download_category');	//下级类别的UId
	$CurUId=$db->get_value('download_category', "CateId='$CateId'", 'UId');
	$db->query("update download_category set UId=replace(UId, '$CurUId', '$UId') where UId like '{$SubUId}%'");
	...
	save_manage_log('更新下载类别:'.$Category);
...
$category_description_row=$db->get_one('download_category_description', "CateId='$CateId'");

include('../../inc/manage/header.php');
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="category.php"><?=get_lang('download.category_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.mod');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	...
	<?php }?>
	<?php
	if(get_cfg('download.category.dept')>1){
		$o_dept=$category_row['Dept']-($db->get_max('download_category', "UId like '{$category_row['UId']}{$category_row['CateId']},%'", 'Dept')-get_cfg('download.category.dept'));	//剩余可用的级数
		$e_where="CateId!='{$category_row['CateId']}' and Dept<".($category_row['SubCate']?$o_dept:get_cfg('download.category.dept'));
	?>
		<tr>
			<td nowrap><?=get_lang('ly200.under_the');?>:</td>
			<td><?=ouput_Category_to_Select('UnderTheCateId', get_CateId_by_UId($category_row['UId']), 'download_category', "UId='0,' and $e_where", $e_where, get_lang('ly200.select'));?></td>
		</tr>
	<?php }?>
